...
const COUNTRTY_MAXIMUM_COUNT = 220 as const;

export const getCountryFlag = async (): Promise<CountryFlagItem[]> => {
  const response = await fetch(
    `${process.env.NEXT_PUBLIC_COUNTRYFLAG_API_URL}?ServiceKey=${process.env.COUNTRYFLAG_SERVICE_KEY}&numOfRows=${COUNTRTY_MAXIMUM_COUNT}`
  );
